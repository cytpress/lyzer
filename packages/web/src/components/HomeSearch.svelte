<script lang="ts">
  ...
  let searchIndexes = $state<MiniSearch[]>([]);
  let searchChunks = $state<string[]>([]);
  ...
  const results = $derived.by(() => {
    ...
    if (query && sortOrder === "relevance") return scoped;
  ...
  const status = $derived(
    waitingForCatalog
      ...
      : loadingCatalog
        ...
        : loadingSearch
          ? searchChunks.length
            ? `載入搜尋索引中（${searchIndexes.length}/${searchChunks.length}）`
            : "載入搜尋索引中"
          : searchLoadFailed && query
            ? "搜尋索引載入中斷，結果可能不完整"
            : query
              ? searchChunks.length && searchIndexes.length < searchChunks.length
                ? `搜尋「${query}」（結果持續載入中）`
                : `搜尋「${query}」`
              : ""
  );
  ...
  async function loadSearch(): Promise<void> {
    if (searchChunks.length > 0 && searchIndexes.length === searchChunks.length) return;
    ...
    searchPromise = (async () => {
      ...
      try {
        ...
        // 逐塊顯示已可用的結果，不等所有大型索引下載完才回應。
        while (searchIndexes.length < searchChunks.length) {
          const chunkResponse = await fetch(searchChunks[searchIndexes.length]);
          ...
          searchIndexes = [...searchIndexes, MiniSearch.loadJSON(chunkPayload.index, miniSearchOptions)];
        }
      } catch (error) {
        console.warn(error);
  ...
  <div class="page-shell mb-6 flex flex-col gap-4">
    ...
    <div class="flex min-h-10 items-center justify-between gap-3 text-sm text-neutral-500">
      ...
      <div class="flex items-center gap-2">
        ...
        {#if searchLoadFailed && query}
          <button class="underline hover:text-neutral-900" type="button" onclick={() => void loadSearch()}>重試</button>
        {/if}
        {#if query}
