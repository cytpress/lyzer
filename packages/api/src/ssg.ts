...
import type { AgendaDetail, HomepageAgenda, JsonObject } from "./types.js";
...
export async function getCommittees(): Promise<string[]> {
  ...
  return Array.from(new Set(agendas.map((agenda) => agenda.committee).filter((item): item is string => Boolean(item)))).sort(
