...
4.  **agenda_items (物件陣列)：** 從**實際文本中**識別並**詳細列出「主要」議程項目**。對於每個議程項目，根據**實際文本內容**提供：
    ...
    *   **core_issue (字串陣列 或 null)：** 將該議程項目討論的「所有核心議題、主要關切點、背景及關鍵面向」，「高度概括並綜合提煉」成「一句或最多兩句」能涵蓋整體討論焦點的描述性語句。應著重於議題的「實質內容和主題方向」，避免僅列舉細項或程序性描述。如果確實存在多個「重大且獨立」的議題方向（例如討論完一個法案後接著討論另一個完全不同的法案），才可作為陣列中的不同元素（總數不宜超過二個主要方向），但每個元素仍需是高度概括性的。無明確核心議題則設為 null。
    *   **controversy (字串陣列 或 null)：** 務必清晰且具體地描述該議程項目「最主要」的「至多10個」關鍵爭議點。對於每個爭議點，應明確呈現至少兩方（或多方）的「核心」不同觀點、論點或具體行為差異，並「簡述其主要理由」。**嚴格篩選，避免列出超過三個以上的爭議點，即使文本中提及多個潛在分歧。僅選擇那些對議案走向或政策辯論具有實質影響的、最突出的分歧點。** 無明顯重大爭議則為 null。
    ...
    *   **respondent_speakers (物件陣列 或 null)：** 列出在該議程項目中「最主要回應或報告」的「政府官員」或「相關事業單位代表」。
        ...
        *   **speaker_viewpoint (字串陣列 或 null)：** **高度精煉並直接陳述**該答詢者針對質詢的「至多10個最核心的政策回應、明確說明、關鍵數據提供、官方立場或承諾」。**「嚴格篩選」，避免羅列所有答覆細節或重複性澄清。直接點出其實質答覆內容。** 若有多個獨立且「關鍵」的實質答覆點（總數不宜超過三個），作為陣列元素。完全排除純粹的程序性發言。
    *   **result_status_next (字串陣列 或 null)：** 清晰且相對完整地說明關於此議程的「具有實質意義的最終處理結果、審查進度或明確的下一步行動」。**務必排除僅為程序性宣告（如「相關人員可先行離席」、「宣讀完畢」、「繼續討論下一案」、「某委員提案不處理/撤案」等與議案實質進展無關的描述）或無明確決議的內容。應「總結性地」描述議案的整體處置，例如「多項預算提案經討論後部分凍結共計XX元，要求提交書面報告；部分提案改為主決議，要求針對XX議題進行改善。」而非逐一列出每個提案的處理細節。僅記錄影響議案走向或具有約束力的「關鍵決議」（如法案送交何處、重大預算調整）、交付事項、重要的保留意見及原因、或確切的後續步驟。** 若有多個獨立的實質結果或行動（總數不宜超過5個關鍵點），作為陣列元素。若無明確的實質結果或下一步則設為 null。
