/**
 * Determines if analysis should be skipped based on the provided category code.
 * @param categoryCode The category code of the content.
...
export function shouldSkipAnalysis(
  ...
  // Example: Skip category 7. Adjust this based on your specific category meanings.
  return categoryCode === 7;
}

// --- Main Analysis Prompt ---
/**
 * Generates the primary analysis prompt for the Gemini API.
 * This prompt is designed for detailed extraction and summarization.
 * All instructional text within the prompt is in Traditional Chinese.
 * @param categoryCode The category code, used to provide contextual hints within the prompt.
 * @param textContent The actual text content to be analyzed.
 * @param isTruncated Boolean flag indicating if the `textContent` was truncated due to length.
 * @returns The fully constructed prompt string in Traditional Chinese.
 */
export function getAnalysisPrompt(
  ...
  // Logic to set contextHint and committeeNameInstruction
  if (
    categoryCode === 1 ||
    categoryCode === 2 ||
    categoryCode === 4 ||
...
// --- Shortened Analysis Prompt ---
/**
 * Generates a more concise analysis prompt for the Gemini API.
 * This is typically used for retries, aiming for shorter AI output.
 * All instructional text within the prompt is in Traditional Chinese.
 * @param categoryCode The category code for contextual hints.
 * @param textContent The actual text content to be analyzed.
 * @param isTruncated Boolean flag indicating if `textContent` was truncated.
 * @param previousErrorHint A hint about why the previous analysis attempt might have failed.
 * @returns The fully constructed shortened prompt string in Traditional Chinese.
 */
